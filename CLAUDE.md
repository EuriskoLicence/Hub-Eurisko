...
- **OdA line options**: `getOdaLinesForClient(clientId)` returns the client's PO lines EXCLUDING those with status code `'INV'` (fattura emessa). Lines already matched by the current invoice but now in INV status are still shown via `getReferencedInvoicedOdaLines` (label suffixed "[Fatturata]", option disabled). Additional filters (e.g. residual amount to invoice) intentionally deferred.
